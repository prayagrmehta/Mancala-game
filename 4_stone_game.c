...
int binamount[] = {4, 4, 4, 4, 4, 4, 0, 4, 4, 4, 4, 4, 4, 0};
bool playerOne = true;
GtkWidget *labels[14];
...
void reset_game() {
    int initial_bins[] = {4, 4, 4, 4, 4, 4, 0, 4, 4, 4, 4, 4, 4, 0};
    for (int i = 0; i < 14; i++) {
        binamount[i] = initial_bins[i];
    }
    playerOne = true;
    update_board();
...
void check_game_end() {
    ...
    if (playerOneBins == 0 || playerTwoBins == 0) {
        ...
        if (binamount[6] > binamount[13]) {
            gtk_label_set_text(GTK_LABEL(statusLabel), "Game Over! Player 1 Wins!");
        } else if (binamount[13] > binamount[6]) {
            gtk_label_set_text(GTK_LABEL(statusLabel), "Game Over! Player 2 Wins!");
        } else {
            gtk_label_set_text(GTK_LABEL(statusLabel), "Game Over! It's a Draw!");
        }

        // Disable further moves (disable reset button)
        gtk_widget_set_sensitive(resetButton, TRUE); // Enable reset button for a new game
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // Create a reset button and add it to the vbox
    resetButton = gtk_button_new_with_label("Reset Game");
    gtk_box_pack_start(GTK_BOX(vbox), resetButton, FALSE, FALSE, 0);
    g_signal_connect(resetButton, "clicked", G_CALLBACK(reset_game), NULL);
    gtk_widget_set_sensitive(resetButton, TRUE); // Initially enabled
